...
public class ArrowSlitBlock extends VerticalConnectingFacingBlock {
    ...
    public ArrowSlitBlock(Settings settings) {
        super(settings);
        setDefaultState(getDefaultState().with(Properties.HORIZONTAL_FACING, Direction.NORTH).with(ModProperties.VERTICAL_CONNECTED, VerticalConnected.SINGLE).with(Properties.WATERLOGGED, Boolean.FALSE).with(VARIATIONS, 1));
    }

    protected void appendProperties(StateManager.Builder<Block, BlockState> builder)
    {
        builder.add(Properties.HORIZONTAL_FACING, ModProperties.VERTICAL_CONNECTED, Properties.WATERLOGGED, VARIATIONS);
    }
